...
def minimize_structure(atoms, calculator, fmax=0.01, steps=1000):
    """
...
Folders = {
    ...
    "llamat3": "/scratch/scai/phd/aiz238703/MDBENCHGNN/mace_universal_2.0/Visualisation/VisCode/llamat3_14000_cif/"
}

# Initialize output and file writing
counter = 1
file_write = open("llamat2_MACE_rlx_Engy.txt", 'w')

# Loop through structures in the folder, minimize and compute energy
for file in tqdm(os.listdir(Folders["llamat2"])):
    counter += 1
    try:
        # Load structure
        LoadedStructure = ase.io.read(os.path.join(Folders["llamat2"], file))
        
        # Set the calculator and minimize structure
        minimized_structure, traj = minimize_structure(LoadedStructure, calculator, fmax=0.01, steps=1000)
