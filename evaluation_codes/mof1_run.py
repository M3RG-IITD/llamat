...
# Output will be stored in the same folder as <SAVE_NAME_PREFIX>_mof1_test.pkl
import os
import json
import pandas as pd
import random
...
path = sys.argv[2]; # The path is the whole path in this case.
model_dir = path
...
model.eval()
with torch.no_grad():
    print(tokenizer.decode(model.generate(**model_input, top_p=0.95, max_length= min(int(model_input['input_ids'].shape[1]+ out_tokens[idx]+100),2048),do_sample=False)[0], skip_special_tokens=True))
...
for sample in tqdm(mof1_test[:]):
    ...
    if model_input['input_ids'].shape[1]>=2048:
        ...
        output = model.generate(**model_input, **generation_config, temperature = 0.05,  top_p=0.5, max_length= min(int(model_input['input_ids'].shape[1]+ out_tokens[idx]+400),2048),do_sample=False) # for atom count and atom name
    
        generated_tokens = output[0, model_input['input_ids'].shape[1]:]
        generated_text = tokenizer.decode(generated_tokens, skip_special_tokens=True) # Original
        # generated_text = tokenizer.decode(generated_tokens, skip_special_tokens=False) # Test
...
if len(sys.argv) > 3:
    savepath = sys.argv[3] + '_mof1_test.pkl'
    print("*"*20, "\nsavepath is ", savepath)
else:
    savepath = sys.argv[2].split('/')[-3] + '_mof1_test.pkl';
    if('cs12' in savepath or len(savepath) <= 15):
        x = sys.argv[2].split('/')
        savepath = 'llama_basic_' + x[-1] + '_mof1_test.pkl';
