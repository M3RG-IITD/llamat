    ...
    if iteration % args.log_interval == 0:
        ...
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key,
                ...
                if avg > 0.0:
                    # with open('/home/cse/btech/cs1200389/train_errors.txt', 'a') as f:
                    #     f.write(f'{iteration}/{args.train_iters}: training loss = {avg}\n')
        ...
        # if get_args().local_rank == (int(os.environ["LOCAL_WORLD_SIZE"]) - 1):
        #     with open('/home/cse/btech/cs1200389/train_logs.txt', 'a') as f:
        #         f.write(log_string+'\n')            
...
def evaluate_and_print_results(prefix,
    ...
    for key in total_loss_dict:
        ...
        if "lm loss" in key:
            ...
            # with open('/home/cse/btech/cs1200389/val_errors.txt', 'a') as f:
            #     f.write(string + '\n')
        else:
            ppl = None
        if writer:
...
def test_and_print_results(prefix,
                               forward_step_func,
                               data_iterator, model,
                               iteration, process_non_loss_data_func,
                               verbose=False,
    ...
    total_loss_dict, collected_non_loss_data = evaluate(
        forward_step_func, data_iterator, model,
        process_non_loss_data_func, verbose)
    string = ' test loss at {} | '.format(prefix)
    for key in total_loss_dict:
        string += '{} value: {:.6E} | '.format(key, total_loss_dict[key].item())
        if "lm loss" in key:
            ppl = math.exp(min(20, total_loss_dict[key].item()))
            string += '{} PPL: {:.6E} | '.format(key, ppl)
            # with open('/home/cse/btech/cs1200389/test_errors.txt', 'a') as f:
            #     f.write(string + '\n')
        else:
            ppl = None
        if writer:
